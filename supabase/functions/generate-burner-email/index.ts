...
// ============= Alias generation constants =============
const ALIAS_SYLLABLES = [
...
const BLACKLISTED_WORDS = [
  "admin","support","root","test","mail","abuse","owner","help",
  "postmaster","webmaster","spam","info","noreply","null","void",
];

/**
 * Generates a short, human-readable alias: 2-3 syllables + 2 digits.
 * Format: e.g. mika47, zoru18, laven62
 * Local-part length: 6-10 chars (4-8 letters + 2 digits).
 */
function generateRandomEmail(): string {
  const syllableCount = 2 + Math.floor(Math.random() * 2); // 2 or 3 syllables
  let letters = '';
  for (let i = 0; i < syllableCount; i++) {
    letters += ALIAS_SYLLABLES[Math.floor(Math.random() * ALIAS_SYLLABLES.length)];
  }
  const digits = Math.floor(Math.random() * 90 + 10).toString(); // 10-99
  return `${letters}${digits}@burner.privaseer.co.uk`;
}

/**
 * Validates a generated alias against all quality rules.
 * Returns true only if the alias passes all checks.
 */
function isValidAlias(email: string): boolean {
  const local = getLocalPart(email);

  // Rule 2: Length 6-12
  if (local.length < 6 || local.length > 12) return false;

  // Rule 4: Must contain at least 1 letter and 1 digit
  if (!/[a-z]/.test(local) || !/[0-9]/.test(local)) return false;

  // Rule 7: No character repeated more than 3 times consecutively
  if (/(.)\1{3,}/.test(local)) return false;

  // Rule 8: No blacklisted words
  for (const word of BLACKLISTED_WORDS) {
    if (local.includes(word)) return false;
  }

  // Rule 9: Must be lowercase (already enforced by generator)
  if (local !== local.toLowerCase()) return false;

  return true;
}

/**
 * Rule 6: Detects if candidate is a sequential variant of any existing alias.
 * e.g., alex01 when alex00 exists, or abc124 when abc123 exists.
 */
function isSequentialVariant(candidateEmail: string, existingLocalParts: string[]): boolean {
  const candidate = getLocalPart(candidateEmail);

  // Extract trailing number from candidate
  const candidateMatch = candidate.match(/^([a-z]+)(\d+)$/);
  if (!candidateMatch) return false;

  const candidatePrefix = candidateMatch[1];
  const candidateNum = parseInt(candidateMatch[2], 10);

  for (const existing of existingLocalParts) {
    const existingMatch = existing.match(/^([a-z]+)(\d+)$/);
    if (!existingMatch) continue;

    if (existingMatch[1] === candidatePrefix) {
      const existingNum = parseInt(existingMatch[2], 10);
      if (Math.abs(candidateNum - existingNum) <= 1) {
        return true; // Sequential: same prefix, numbers differ by 0 or 1
      }
    }
  }
  return false;
...
/**
 * Extracts the local-part (before @) from an email address, lowercased.
 */
...
/**
 * Calculates positional mismatch ratio between two strings.
 * Returns a value between 0 (identical) and 1 (completely different).
 */
function calculateMismatch(a: string, b: string): number {
  ...
  return 1 - sameChars / maxLen;
}

/**
 * Returns true if candidate local-part has >= 50% mismatch with ALL existing local-parts.
 */
function hasEnoughMismatch(candidateEmail: string, existingLocalParts: string[]): boolean {
  const candidateLocal = getLocalPart(candidateEmail);
  for (const existing of existingLocalParts) {
    if (calculateMismatch(candidateLocal, existing) < 0.50) {
      return false;
    }
  }
  return true;
...
/**
 * Full validation: unique format + mismatch + no sequential.
 */
function isFullyValid(email: string, existingLocalParts: string[]): boolean {
  return isValidAlias(email)
    && hasEnoughMismatch(email, existingLocalParts)
    && !isSequentialVariant(email, existingLocalParts);
}
...
denoRuntime.serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      // Fetch existing local-parts for mismatch comparison (recent 200)
      ...
      const existingLocalParts = (existingEmails ?? [])
        .map((e: { email_address: string }) => getLocalPart(e.email_address))
        .filter((lp: string) => lp.length > 0);
      ...
      while (attempts < maxAttempts) {
        ...
        if (!existing && isFullyValid(emailAddress, existingLocalParts)) {
          break;
        }
      ...
      // Fallback: use 3 syllables + 3 digits for more entropy
      if (attempts === maxAttempts) {
        ...
        for (let i = 0; i < 10; i++) {
          ...
          emailAddress = `${letters}${digits}@burner.privaseer.co.uk`;
          ...
          if (!dup && isFullyValid(emailAddress, existingLocalParts)) {
