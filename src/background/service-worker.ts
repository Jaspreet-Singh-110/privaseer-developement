...
import { initAlternativeEngine, classifyAndSuggest } from './alternative-engine';
...
const tabConsentRejectionCache = new Map<number, { domain: string; timestamp: number }>();
...
export function getTabConsentRejection(tabId: number, domain: string): number | null {
  const entry = tabConsentRejectionCache.get(tabId);
  if (!entry || entry.domain !== domain) {
    return null;
  }
  if (Date.now() - entry.timestamp > CONSENT_VIOLATION.REJECTION_WINDOW_MS) {
    tabConsentRejectionCache.delete(tabId);
    return null;
  }
...
TrackerDetector.setConsentRejectionProvider(getTabConsentRejection);
...
// Deduplication cache: prevents the same domain from flooding the alert list
const behavioralAlertCache = new Map<string, number>();

function setupBehavioralTrackerAlerts(): void {
  backgroundEvents.on('BEHAVIORAL_TRACKER_DETECTED', async ({ tracker }) => {
    try {
      // Deduplicate: only alert once per domain per hour
      const lastAlertTime = behavioralAlertCache.get(tracker.domain);
      const now = Date.now();
      if (lastAlertTime && now - lastAlertTime < 3600000) return;
      behavioralAlertCache.set(tracker.domain, now);

      const severity: 'low' | 'medium' | 'high' =
        tracker.riskLevel === 'High risk' ? 'high' :
          tracker.riskLevel === 'Medium risk' ? 'medium' : 'low';

      await Storage.addAlert({
        id: `behavioral-${now}-${Math.random()}`,
        type: 'high_risk',
        severity,
        message: `${tracker.domain} — ${tracker.reason}`,
      ...
      logger.info('ServiceWorker', 'Behavioral tracker alert added to dashboard', {
        domain: tracker.domain,
        riskLevel: tracker.riskLevel,
      });
    } catch (error) {
      logger.error('ServiceWorker', 'Failed to create behavioral tracker alert', toError(error));
    }
  });
}

// Deduplication cache for violations
const violationAlertCache = new Map<string, number>();

function setupComplianceViolationAlerts(): void {
  backgroundEvents.on('COMPLIANCE_VIOLATION_DETECTED', async ({ violation }) => {
    try {
      const domain = violation.tracker.domain;
      const now = Date.now();

      // Deduplicate: only alert once per domain per hour
      const lastAlertTime = violationAlertCache.get(domain);
      if (lastAlertTime && now - lastAlertTime < 3600000) return;
      violationAlertCache.set(domain, now);

      await Storage.addAlert({
        id: `violation-${now}-${Math.random()}`,
        type: 'post_consent_violation',
        severity: 'high',
        message: `${domain} tracked you after you rejected consent`,
        domain,
        timestamp: now,
        url: violation.tracker.tabUrl,
        blockedTrackers: [domain],
        trackerCount: 1,
      });

      messageBus.broadcast('STATE_UPDATE');
      logger.warn('ServiceWorker', 'Consent compliance violation alert added to dashboard', { domain });
    } catch (error) {
      logger.error('ServiceWorker', 'Failed to create compliance violation alert', toError(error));
    }
  });
}
...
async function initializeExtension(): Promise<void> {
  ...
  initializationPromise = (async () => {
    try {
      // Chrome MV3: register all listeners synchronously first
      setupMessageHandlers();
      setupStorageChangeListener();
      setupTabEventHandlers();
      setupCleanupInterval();

      // Phase 1: utilities in parallel
      await Promise.all([
        messageBus.initialize(),
        tabManager.initialize(),
      ]);

      // Phase 2: score manager (depends on utilities)
      await PrivacyScoreManager.initialize();

      // Phase 3: core components in parallel
      await Promise.all([
        Storage.initialize(),
        FirewallEngine.initialize(),
        burnerEmailService.initialize(),
        feedbackTelemetryService.initialize(),
      ]);

      // Phase 4: sync inits + badge (no await needed for these)
      TrackerDetector.initialize();
      setupBehavioralTrackerAlerts();
      setupComplianceViolationAlerts();

      // Phase 5: all background network fetches fire together (non-blocking)
      void Promise.all([
        fetchFalsePositiveOverrides(),
        fetchCmpConfig(),
        fetchScoringConfig(),
        initAlternativeEngine(),
      ]);
...
function setupMessageHandlers(): void {
  ...
  messageBus.on('GET_ALTERNATIVES', async (data: unknown) => {
    try {
      ...
      const result = await classifyAndSuggest(domain, 'site');
      return {
        success: true,
      ...
      logger.error('ServiceWorker', 'Failed to get alternatives', toError(error));
      return { success: false, error: 'Failed to get alternatives' };
    }
  });

  messageBus.on('CLASSIFY_RISK', async (data: unknown) => {
    try {
      const { domain, category, trackerCount } = data as {
        domain: string;
        category: string;
        trackerCount?: number;
      };
      if (!domain || !category) {
        return { success: false, error: 'Invalid classify risk payload' };
      }

      const result = await classifyAndSuggest(domain, category, trackerCount ?? 1);
      return {
        success: true,
        riskLevel: result.riskLevel,
        dataType: result.dataType,
      ...
      logger.error('ServiceWorker', 'Failed to classify risk', toError(error));
      return { success: false, error: 'Failed to classify risk' };
    }
  });
  ...
  messageBus.on('CONSENT_REJECT_CLICKED', async (data: unknown, sender) => {
    const payload = data as { domain: string };
    if (sender?.tab?.id && payload?.domain) {
      tabConsentRejectionCache.set(sender.tab.id, {
        domain: payload.domain,
        timestamp: Date.now()
      });
      logger.info('ServiceWorker', 'Tab-level consent rejection recorded', {
        domain: payload.domain,
        tabId: sender.tab.id,
      });
    }
...
function setupTabEventHandlers(): void {
  ...
  // Broadcast PRIVACY_ADVISOR_ALERT to content scripts when trackers are blocked
  backgroundEvents.on('TRACKER_BLOCKED', (eventData) => {
    const { domain: trackerDomain, category, tabId } = eventData;
    if (tabId > 0) {
      chrome.tabs.get(tabId, (tab) => {
        ...
        try {
          ...
          chrome.tabs.sendMessage(
            tabId,
            {
              type: 'PRIVACY_ADVISOR_ALERT',
              data: { trackerDomain, category, siteDomain },
            },
            () => {
              if (chrome.runtime.lastError) {
