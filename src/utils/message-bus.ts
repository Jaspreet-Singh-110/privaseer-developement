...
const payloadValidators: Partial<Record<MessageType, (payload: unknown) => boolean>> = {
  ...
  REMOVE_FROM_ALLOWLIST: (data): data is MessageDataMap['REMOVE_FROM_ALLOWLIST'] =>
    isObject(data) && typeof (data as { domain?: unknown }).domain === 'string',
  GET_ALTERNATIVES: (data): data is MessageDataMap['GET_ALTERNATIVES'] =>
    isObject(data) && typeof (data as { domain?: unknown }).domain === 'string',
  CLASSIFY_RISK: (data): data is MessageDataMap['CLASSIFY_RISK'] =>
    isObject(data) &&
    typeof (data as { domain?: unknown }).domain === 'string' &&
    typeof (data as { category?: unknown }).category === 'string',
  PRIVACY_ADVISOR_ALERT: (data): data is MessageDataMap['PRIVACY_ADVISOR_ALERT'] =>
    isObject(data) &&
    typeof (data as { trackerDomain?: unknown }).trackerDomain === 'string' &&
    typeof (data as { category?: unknown }).category === 'string' &&
    typeof (data as { siteDomain?: unknown }).siteDomain === 'string',
};
