...
class ConsentScanner {
  ...
  private setupGlobalEventDelegation() {
    document.addEventListener('click', (event) => {
      ...
      const acceptKeywords = ['accept', 'allow all', 'agree', 'got it', 'i understand', 'allow'];
      
      const isRejectAction = rejectKeywords.some(k => combinedText.includes(k));
      const isAcceptAction = !isRejectAction && acceptKeywords.some(k => combinedText.includes(k));

      if (isRejectAction) {
        console.log('[ConsentScanner] Reject click detected:', combinedText);

        this.showConsentToast('Consent Rejected – Monitoring tracking activity');
        void chrome.storage.local.set({ [`consent_action_${window.location.hostname}`]: { action: 'Rejected', timestamp: Date.now() } });
        ...
        console.log('[ConsentScanner] Accept click detected:', combinedText);
        this.showConsentToast('Consent Accepted – No blocking enforced', false, true);
        void chrome.storage.local.set({ [`consent_action_${window.location.hostname}`]: { action: 'Accepted', timestamp: Date.now() } });
      }
    }, true);
  }
  ...
  private showConsentToast(message: string, isViolation: boolean = false, isAccept: boolean = false): void {
    ...
    notification.innerHTML = `
      <div style="display: flex; align-items: center; gap: 8px;">
        ...
        <div>
          <div style="font-weight: 600; margin-bottom: 2px;">Privaseer</div>
    ...
    notification.style.cssText = `
      ...
      right: 20px;
      padding: 16px 20px;
      background: ${isViolation ? '#ef4444' : isAccept ? '#10B981' : 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)'};
      color: white;
      border-radius: 8px;
      font-size: 14px;
      z-index: 2147483647;
